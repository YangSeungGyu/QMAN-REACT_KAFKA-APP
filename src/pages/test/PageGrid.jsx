...
import { pageGridListRequest } from 'src/features/test/PageGridActions'; 

ModuleRegistry.registerModules([AllCommunityModule]);

function PageGrid() {
    const dispatch = useDispatch();
    const { list, totalCnt, loading, error } = useSelector(state => state.pageGrid);
    ...
    const handleSearch = () => {
      setPage(1);
      dispatch(pageGridListRequest(1, size, searchTitle, searchWriter));
    };


    useEffect(() => {
      dispatch(pageGridListRequest(page, size));
    }, [dispatch, page]);
 ...
 <style>
  {`
    ...
    .page-grid-paging .page-size-wrap {
      margin-right: 12px;
    }

    .page-grid-paging .page-size-wrap select {
      margin-left: 4px;
      font-size: 13px;
    }

    .page-grid-paging .page-info {
      margin-right: 12px;
    }

    .page-grid-paging button {
      border: none;
      background: none;
      cursor: pointer;
      font-size: 14px;
    ...
    .page-grid-paging button:disabled {
      color: #ccc;
      cursor: default;
    }
